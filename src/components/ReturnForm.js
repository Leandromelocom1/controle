...
const ReturnForm = ({ refreshTools }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (defective) {
        alert('Ferramenta devolvida com defeito e enviada para manutenção.');
      } else {
        alert('Ferramenta devolvida com sucesso e retornada ao estoque.');
      }
    } catch (error) {
        ...
        >
          ...
          {tools.map(tool => (
            <option key={tool._id} value={tool._id}>
              {tool.toolName} - {tool.serialNumber} ({tool.status})
            </option>
          ))}
        </select>
      </div>
